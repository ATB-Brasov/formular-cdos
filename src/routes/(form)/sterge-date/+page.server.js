...
export const actions = {
    delete: async ({ request }) => {
        ...
        if (email == null || answerId == null) {
            return fail(400, { deleteMsg: "Introdu atât adresa de e-mail cât și ID-ul răspunsului." });
        }
        ...
        if (!result.deleted) {
            if (!result.emailExists && !result.answerExists) {
                return fail(404, { deleteMsg: "Nu s-au găsit date pentru această adresă și acest ID." });
            }
            return fail(400, { deleteMsg: "Adresa de e-mail și ID-ul nu corespund. Verifică datele introduse." });
        }
