...
<div class="py-18 w-full flex flex-col gap-y-12 text-center">
    ...
    <p class="text-lg">
        Pentru a-ți șterge răspunsurile, introdu adresa de e-mail și ID-ul răspunsului primit după completare.
    </p>

    <form method="POST" action="?/delete" use:enhance class="mx-auto max-w-md flex flex-col gap-3">
        <input
            type="text"
            name="answerId"
            value={data.answerId ?? ''}
        ...
        >
        <button type="submit" class="self-end rounded-md px-4 py-1.5 bg-danger text-white text-sm hover:bg-danger-hover">
            Șterge răspunsurile
        </button>
