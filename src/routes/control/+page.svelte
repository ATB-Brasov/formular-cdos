<script>
    ...
    let chartData = $derived.by(() => {
        const sortedEntries = Array.from(answersPerDay.entries()).sort(([dateA], [dateB]) => {
            return new Date(dateA.split('.').reverse().join('-')).valueOf() - new Date(dateB.split('.').reverse().join('-')).valueOf();
        });
    ...
    $effect(() => {
        if (chartData.labels.length > 0 && chartContainer) {
            const renderChart = () => {
                ...
                svgChart.setAttribute("viewBox", `0 0 ${chartAreaWidth} ${chartAreaHeight}`);
                ...
                for (let i = 0; i < numberOfGridLines; i++) {
                    const gridLineY = margin.top + (plotHeight / (numberOfGridLines - 1)) * i;
                    const line = document.createElementNS(svgNS, "line");
                    line.setAttribute("x1", String(margin.left));
                    line.setAttribute("y1", String(gridLineY));
                    line.setAttribute("x2", String(chartAreaWidth - margin.right));
                ...
                labels.forEach((label, i) => {
                    ...
                    const x = margin.left + (totalBarSlotWidth * i) + (spacing / 2);
                    ...
                    text.setAttribute("y", String(chartAreaHeight - margin.bottom + 15));
...
{:else}
    <div class="container mx-auto p-4 md:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <!-- Total Answers Card -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 flex flex-col items-center justify-center border border-gray-200 dark:border-gray-700 transition-shadow duration-300 hover:shadow-lg">
                <h2 class="text-lg font-semibold text-gray-600 dark:text-gray-300 mb-2">Total Answers Submitted</h2>
                <p class="text-5xl font-bold text-blue-600 dark:text-blue-400">{totalAnswers}</p>
            </div>
        </div>

        <!-- Answers Per Day Chart -->
        {#if chartData.labels.length > 0}
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8 border border-gray-200 dark:border-gray-700 transition-shadow duration-300 hover:shadow-lg">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Daily Submissions</h2>
                <div bind:this={chartContainer} style="height: 300px; position: relative;">
        ...
        {:else if data.answers != null}
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8 border border-gray-200 dark:border-gray-700">
                <p class="text-gray-600 dark:text-gray-300">No daily answer data available to display.</p>
            </div>
        {/if}
        ...
        {:else if data.answers != null}
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-700">
                <p class="text-gray-600 dark:text-gray-300">No answers to display.</p>
            </div>
        {/if}
    </div>
{/if}
